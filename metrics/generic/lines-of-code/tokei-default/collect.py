...
def parse_tokei_json(raw_output):
    ...
    if isinstance(total_block, dict):
        code_value = total_block.get("code")
        if isinstance(code_value, (int, float)):
            return int(code_value)

    total = 0
    for language_name, data in payload.items():
        if language_name == "Total":
            continue
...
def collect_module_value(module_path, dry_run):
    ...
    try:
        if copied == 0:
            return 0
        output = run_command([TOOL_NAME, "--output", "json", staging_dir], dry_run)
        if dry_run:
            return 0
        return parse_tokei_json(output)
    finally:
...
def main():
    parser = argparse.ArgumentParser(description="Collect lines-of-code using tokei.")
