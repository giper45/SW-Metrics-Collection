...
Module-level lines-of-code using tokei (code lines only).

## Build

```bash
docker build -t loc-tokei-default:latest metrics/generic/lines-of-code/tokei-default
```
...
docker run --rm \
  -v "$(pwd)/src:/app:ro" \
  -v "$(pwd)/results:/results" \
  loc-tokei-default:latest --dry-run
```

## Output file pattern

`/results/<project>-<timestamp>-lines-of-code-tokei-default.jsonl`
