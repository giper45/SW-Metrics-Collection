...
Module-level lines-of-code using cloc (code lines only).

## Build

```bash
docker build -t loc-cloc-default:latest metrics/generic/lines-of-code/cloc-default
```
...
docker run --rm \
  -v "$(pwd)/src:/app:ro" \
  -v "$(pwd)/results:/results" \
  loc-cloc-default:latest --dry-run
```

## Output file pattern

`/results/<project>-<timestamp>-lines-of-code-cloc-default.jsonl`
