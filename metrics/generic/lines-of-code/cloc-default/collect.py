...
def parse_cloc_json(raw_output):
    ...
    sum_section = payload.get("SUM")
    if isinstance(sum_section, dict):
        code_value = sum_section.get("code")
...
def collect_module_value(module_path, dry_run):
    ...
    try:
        if copied == 0:
            return 0
        output = run_command([TOOL_NAME, "--json", "--quiet", "--skip-uniqueness", staging_dir], dry_run)
        if dry_run:
            return 0
        return parse_cloc_json(output)
    finally:
...
def main():
    parser = argparse.ArgumentParser(description="Collect lines-of-code using cloc.")
