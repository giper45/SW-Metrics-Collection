# lizard-prod-and-tests

Module-level mean cyclomatic complexity using lizard (production + tests).

## Build

```bash
docker build -t cc-lizard-prod-and-tests:latest metrics/generic/cyclomatic-complexity/lizard-prod-and-tests
```
...
docker run --rm \
  ...
  cc-lizard-prod-and-tests:latest
```
...
docker run --rm \
  -v "$(pwd)/src:/app:ro" \
  -v "$(pwd)/results:/results" \
  cc-lizard-prod-and-tests:latest --dry-run
```

## Output file pattern

`/results/<project>-<timestamp>-cyclomatic-complexity-lizard-prod-and-tests.jsonl`
