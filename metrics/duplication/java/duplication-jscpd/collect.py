...
def parse_jscpd_ratio(report_path):
    if not os.path.isfile(report_path):
        raise OutputContractError(f"jscpd report missing: {report_path}")

    try:
        payload = json.loads(open(report_path, "r", encoding="utf-8").read())
    except (OSError, json.JSONDecodeError) as exc:
        raise OutputContractError(f"jscpd report is not valid JSON: {report_path}") from exc
    ...
    raise OutputContractError(f"jscpd report missing duplication counters: {report_path}")
...
def collect_module_value(module_path):
    ...
    try:
        ...
        if not os.path.isfile(report_path):
            raise OutputContractError(f"jscpd report missing for module '{module_path}'")
