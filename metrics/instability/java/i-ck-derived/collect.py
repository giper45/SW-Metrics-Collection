...
def compute_instability_from_rows(rows):
    values = []
    for row in rows:
        ce_raw = row.get("ce", row.get("fanout", "0"))
        ca_raw = row.get("ca", row.get("fanin", "0"))
        try:
            ce_value = float(ce_raw or 0)
            ca_value = float(ca_raw or 0)
        except ValueError:
            continue
        denom = ce_value + ca_value
        values.append(0.0 if denom == 0 else ce_value / denom)
...
def collect_module_value(module_path, dry_run):
    ...
    try:
        ...
        return compute_instability_from_rows(rows)
    finally:
...
def main():
    parser = argparse.ArgumentParser(description="Collect module-level instability derived from CK")
